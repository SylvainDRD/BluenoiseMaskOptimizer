...
Display::Display(int dimension, GLuint maskTexture)
    : m_program(buildShaders({PROJECT_ROOT "shaders/display.vert", PROJECT_ROOT "shaders/display.frag"},
                             {GL_VERTEX_SHADER, GL_FRAGMENT_SHADER}, {{"DIMENSION", dimension}})) {
    LOG << "Initializing the display..." << std::endl;
    generateScreenquad();
    ...
    glBindTexture(GL_TEXTURE_2D_ARRAY, maskTexture);
    glUniform1i(glGetUniformLocation(m_program, "mask"), 0);
}
